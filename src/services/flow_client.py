...
class FlowClient:
    ...
    async def _handle_retryable_generation_error(
        ...
        # 500/内部错误退避2s，其他错误退避1s
        backoff = 2.0 if "500" in (retry_reason or "") or "内部错误" in (retry_reason or "") else 1.0
    ...
    async def _sync_json_http_request(
        ...
        def _do_request() -> tuple[int, Optional[Any], str]:
            data = None
