...
class TokenManager:
    ...
    async def add_token(
        ...
        assigned_api_key: Optional[str] = None,
        ...
        normalized_assigned_api_key = assigned_api_key.strip() if assigned_api_key is not None else None
        if normalized_assigned_api_key:
            existing_assigned = await self.db.get_token_by_assigned_api_key(normalized_assigned_api_key)
            if existing_assigned:
                raise ValueError("assigned_api_key 已被其他账号占用")
        ...
        token = Token(
            ...
            name=name,
            remark=remark,
            assigned_api_key=normalized_assigned_api_key,
            is_active=True,
            credits=credits,
    ...
    async def update_token(
        ...
        if assigned_api_key is not None:
            normalized_assigned_api_key = assigned_api_key.strip()
            if normalized_assigned_api_key:
                existing_assigned = await self.db.get_token_by_assigned_api_key(normalized_assigned_api_key)
                if existing_assigned and existing_assigned.id != token_id:
                    raise ValueError("assigned_api_key 已被其他账号占用")
                update_fields["assigned_api_key"] = normalized_assigned_api_key
            else:
                update_fields["assigned_api_key"] = ""
