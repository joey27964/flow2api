...
async def _build_image_parts_from_uri(uri: str, prefer_url: bool = False) -> List[Dict[str, Any]]:
    if prefer_url and (uri.startswith("http://") or uri.startswith("https://")):
        return [
            {
                "fileData": {
                    "mimeType": _guess_mime_type(uri, "image/jpeg"),
                    "fileUri": uri,
                }
            },
        ]
...
async def _build_gemini_parts_from_output(output: str, prefer_url: bool = False) -> List[Dict[str, Any]]:
    if not output:
        return []
    ...
    if image_matches:
        ...
        for uri in image_matches:
            parts.extend(await _build_image_parts_from_uri(uri, prefer_url=prefer_url))
        return parts
...
async def _estimate_image_tokens(output: str) -> int:
    """Estimate completion tokens from image file size via HEAD request."""
    import aiohttp

    # Extract URL from markdown image syntax: ![...](url)
    urls = MARKDOWN_IMAGE_RE.findall(output) if output else []
    if not urls:
        # Try plain URL
        ...
        async with aiohttp.ClientSession() as session:
            for url in urls:
                try:
                    ...
                    total_bytes += 500000  # fallback ~500KB per image
    except Exception:
        return 1000

    if total_bytes > 0:
        # ~10KB per token
        return max(100, total_bytes // 10240)
...
async def _build_gemini_success_payload(
    ...
    # Estimate completion_tokens from image file size
    completion_tokens = await _estimate_image_tokens(output)
        ...
        "candidates": [
            {
                "content": {
                    "role": "model",
                    "parts": await _build_gemini_parts_from_output(output, prefer_url=prefer_url),
                },
                "finishReason": "STOP",
                "index": 0,
            }
...
async def _convert_openai_stream_chunk_to_gemini_event(
    ...
    if text:
        candidate["content"] = {
            "role": "model",
            "parts": await _build_gemini_parts_from_output(text, prefer_url=prefer_url),
        }
    if finish_reason:
...
async def create_chat_completion(
    ...
    try:
        ...
        if request.stream:
            prefer_url = request.model.endswith("-url")
            return StreamingResponse(
                _iterate_openai_stream(normalized, api_key, prefer_url=prefer_url),
...
async def generate_content(
    ...
    try:
        ...
        return JSONResponse(
            content=await _build_gemini_success_payload(payload, model, prefer_url=prefer_url, prompt=normalized.prompt)
        )
...
async def stream_generate_content(
    ...
    try:
        ...
        return StreamingResponse(
            _iterate_gemini_stream(normalized, model, api_key, prefer_url=prefer_url),
            media_type="text/event-stream",
            headers={
                "Cache-Control": "no-cache",
                "Connection": "keep-alive",
