...
class _WALConnection:
    """Async context manager that wraps aiosqlite.connect with WAL + busy_timeout."""
    ...
    async def __aenter__(self):
        self._conn = await aiosqlite.connect(self._db_ref.db_path)
        await self._conn.execute(f"PRAGMA busy_timeout = {self._db_ref.SQLITE_BUSY_TIMEOUT_MS}")
        if not self._db_ref._wal_initialized:
            await self._conn.execute("PRAGMA journal_mode = WAL")
            self._db_ref._wal_initialized = True
        return self._conn
...
class Database:
    ...
    def _open(self):
        """Context manager wrapper for _connect with WAL + busy_timeout."""
        return _WALConnection(self)
    ...
    async def check_and_migrate_db(self, config_dict: dict = None):
        ...
        async with self._open() as db:
            ...
            if await self._column_exists(db, "tokens", "assigned_api_key"):
                await db.execute("CREATE INDEX IF NOT EXISTS idx_tokens_assigned_api_key ON tokens(assigned_api_key)")
    ...
    async def add_token(self, token: Token) -> int:
        ...
        async with self._open() as db:
            cursor = await db.execute("""
                INSERT INTO tokens (st, at, at_expires, email, name, remark, assigned_api_key, is_active,
            ...
            """, (token.st, token.at, token.at_expires, token.email, token.name, token.remark,
                  token.assigned_api_key, token.is_active, token.credits, token.user_paygate_tier,
    ...
    async def has_assigned_api_key(self, api_key: str) -> bool:
        """Check whether API key is assigned to any token."""
        if not api_key:
            return False
        async with self._open() as db:
            cursor = await db.execute(
                "SELECT 1 FROM tokens WHERE assigned_api_key = ? LIMIT 1",
                (api_key,)
            )
            return await cursor.fetchone() is not None

    async def get_assigned_api_keys(self) -> List[str]:
        """List all non-empty assigned API keys."""
        async with self._open() as db:
            cursor = await db.execute(
                """
                SELECT DISTINCT assigned_api_key
                FROM tokens
                WHERE assigned_api_key IS NOT NULL
                  AND TRIM(assigned_api_key) != ''
                """
            )
            rows = await cursor.fetchall()
            return [str(row[0]) for row in rows if row and row[0]]

    async def get_active_token_by_assigned_api_key(self, api_key: str) -> Optional[Token]:
        """Get active token by assigned API key."""
        if not api_key:
            return None
        async with self._open() as db:
            db.row_factory = aiosqlite.Row
    ...
    async def get_token_by_assigned_api_key(self, api_key: str) -> Optional[Token]:
        """Get token (active or inactive) by assigned API key."""
        if not api_key:
            return None
        async with self._open() as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute(
                "SELECT * FROM tokens WHERE assigned_api_key = ? LIMIT 1",
                (api_key,)
            )
            row = await cursor.fetchone()
            if row:
