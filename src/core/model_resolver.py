...
IMAGE_BASE_MODELS = {
    ...
    # Preview aliases (全 b64)
    "gemini-3.1-flash-image-preview": "gemini-3.1-flash-image",
    "gemini-3-pro-image-preview": "gemini-3.0-pro-image",
    # URL variants (全 URL)
    "gemini-3.0-pro-image-url": "gemini-3.0-pro-image",
    "gemini-3.1-flash-image-url": "gemini-3.1-flash-image",
    "gemini-3-pro-image-preview-url": "gemini-3.0-pro-image",
    "gemini-3.1-flash-image-preview-url": "gemini-3.1-flash-image",
    # 2K variants
    "gemini-3-pro-image-preview-2k": "gemini-3.0-pro-image",
    "gemini-3.1-flash-image-preview-2k": "gemini-3.1-flash-image",
    "gemini-3-pro-image-preview-2k-url": "gemini-3.0-pro-image",
    "gemini-3.1-flash-image-preview-2k-url": "gemini-3.1-flash-image",
    # 4K variants
    "gemini-3-pro-image-preview-4k": "gemini-3.0-pro-image",
    "gemini-3.1-flash-image-preview-4k": "gemini-3.1-flash-image",
    "gemini-3-pro-image-preview-4k-url": "gemini-3.0-pro-image",
    "gemini-3.1-flash-image-preview-4k-url": "gemini-3.1-flash-image",
}

def is_url_output_model(model: str) -> bool:
    return "-url" in model

def is_b64_output_model(model: str) -> bool:
    return "preview" in model and "-url" not in model

def _extract_size_from_model_name(model: str):
    if "-4k" in model: return "4k"
    if "-2k" in model: return "2k"
    return None
...
VIDEO_BASE_MODELS = {
    # === User-facing models ===
    "veo3.1-fast": {"landscape": "veo_3_1_t2v_fast_ultra", "portrait": "veo_3_1_t2v_fast_portrait_ultra"},
    "veo3.1": {"landscape": "veo_3_1_t2v_fast_ultra", "portrait": "veo_3_1_t2v_fast_portrait_ultra"},
    "veo3.1-pro": {"landscape": "veo_3_1_t2v_fast_ultra_4k", "portrait": "veo_3_1_t2v_fast_portrait_ultra_4k"},
    "veo3.1-relaxed": {"landscape": "veo_3_1_t2v_fast_ultra_relaxed", "portrait": "veo_3_1_t2v_fast_portrait_ultra_relaxed"},
    # === Internal models ===
    # T2V models
    "veo_3_1_t2v_fast": {
...
_last_output_format = "b64"

def get_last_output_format() -> str:
    return _last_output_format
...
def resolve_model_name(
    ...
    if is_url_output_model(model):
        _last_output_format = "url"
    elif is_b64_output_model(model):
        _last_output_format = "b64_all"
    else:
        _last_output_format = "b64"
    ...
    if model in IMAGE_BASE_MODELS:
        ...
        # Override size from model name if embedded (e.g. xxx-4k, xxx-2k-url)
        embedded_size = _extract_size_from_model_name(model)
        if embedded_size:
...
def get_base_model_aliases() -> Dict[str, str]:
    ...
    for alias, base in IMAGE_BASE_MODELS.items():
        ...
        if "-url" in alias:
            fmt = "all URL"
        elif "preview" in alias:
            fmt = "all base64"
        else:
            fmt = "1K=base64, 2K/4K=URL"
        desc_parts.append(f"output: {fmt}")
