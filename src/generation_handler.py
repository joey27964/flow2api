...
from .r2_handler import upload_to_r2
...
class GenerationHandler:
    ...
    def _resolve_logged_url_from_assets(self, generated_assets: Optional[Dict[str, Any]]) -> Optional[str]:
        """Resolve a stable URL for logs when top-level generated_url is missing."""
        if not isinstance(generated_assets, dict):
            return None

        # Prefer final URLs first
        for key in ("final_video_url", "final_image_url", "origin_image_url"):
            val = generated_assets.get(key)
            if isinstance(val, str) and val.strip():
                return val.strip()

        upscaled = generated_assets.get("upscaled_image")
        if isinstance(upscaled, dict):
            # Prefer HTTP URL for better downstream log display
            for key in ("url", "local_url"):
                val = upscaled.get(key)
                if isinstance(val, str) and val.startswith(("http://", "https://")):
                    return val
            # Fallback to any non-empty value (e.g. data URL)
            for key in ("url", "local_url"):
                val = upscaled.get(key)
                if isinstance(val, str) and val.strip():
                    return val.strip()
    ...
    async def handle_generation(
        ...
        mapped_key_required = False
        if api_key and api_key != config.api_key:
            token = await self.db.get_active_token_by_assigned_api_key(api_key)
            if not token:
                mapped_key_required = await self.db.get_token_by_assigned_api_key(api_key) is not None
            if token:
                debug_logger.log_info(f"[GENERATION] 使用绑定KEY直连Token: {token.id} ({token.email})")
                if generation_type == "image":
                    await self.load_balancer._add_pending(token.id, True, False)
                else:
                    await self.load_balancer._add_pending(token.id, False, True)
                pending_token_state["active"] = True

        if mapped_key_required and not token:
            error_msg = "当前 API Key 未绑定可用账号（账号可能被禁用或失效）"
        ...
        try:
            ...
            if generation_type == "image":
                ...
                async for chunk in self._handle_image_generation(
                    model, token, project_id, model_config, prompt, images, stream,
            ...
            # 添加生成的URL（如果有）
            generated_url = generation_result.get("generated_url") if isinstance(generation_result, dict) else None
            generated_assets = generation_result.get("generated_assets") if isinstance(generation_result, dict) else None
            if not generated_url:
                generated_url = self._resolve_logged_url_from_assets(generated_assets)
            if generated_url:
                response_data["url"] = generated_url
            if generated_assets:
                response_data["generated_assets"] = generated_assets
            ...
            raw_error = str(e)
            status_code = 500
            if "PUBLIC_ERROR_UNSAFE_GENERATION" in raw_error or "PUBLIC_ERROR_MINOR_UPLOAD" in raw_error:
                status_code = 400
                error_msg = "生成失败: Request contains an invalid argument."
            else:
                sanitized = raw_error.replace("Flow API request failed:", "").strip()
                error_msg = f"生成失败: {sanitized}"
            debug_logger.log_error(f"[GENERATION] ❌ {error_msg}")
            if token:
            ...
            if stream:
                yield self._create_stream_chunk(f"❌ {error_msg}\n")
            yield self._create_error_response(error_msg, status_code=status_code)
        finally:
    ...
    async def _handle_image_generation(
        ...
        try:
            ...
            generation_result["generated_assets"] = {
                "type": "image",
                ...
                for retry_attempt in range(max_retries):
                    try:
                        ...
                        if encoded_image:
                            ...
                            generation_result["generated_url"] = image_url
                            generation_result["generated_assets"] = {
                                "type": "image",
                                "origin_image_url": image_url,
                                "upscaled_image": {
                            ...
                            try:
                                ...
                                r2_url = await upload_to_r2(image_bytes, mime_type="image/jpeg")
                            except Exception as upload_error:
                                debug_logger.log_error(f"Failed to upload {resolution_name} image to R2: {str(upload_error)}")
                            ...
                            if config.cache_enabled:
                                try:
                                    ...
                                    if r2_url:
                                        ...
                                        local_url = image_url
                                    generation_result["generated_assets"]["upscaled_image"]["local_url"] = local_url
                                    generation_result["generated_assets"]["upscaled_image"]["url"] = local_url
                                    ...
                                    if stream:
                                        ...
                                        yield self._create_stream_chunk(f"⚠️ 缓存失败: {cache_error}，返回降级链接...\n")

                            # 缓存未启用或缓存失败，返回 base64 格式
                            if url_mode:
                                fallback_url = r2_url or image_url
                                generation_result["generated_assets"]["upscaled_image"]["local_url"] = fallback_url
                                generation_result["generated_assets"]["upscaled_image"]["url"] = fallback_url
                            else:
                                base64_url = f"data:image/jpeg;base64,{encoded_image}"
                                generation_result["generated_assets"]["upscaled_image"]["local_url"] = None
                                generation_result["generated_assets"]["upscaled_image"]["url"] = base64_url
                            self._mark_generation_succeeded(generation_result)
                            if stream:
                                if url_mode:
                                    yield self._create_stream_chunk(
                                        f"![Generated Image]({generation_result['generated_assets']['upscaled_image']['url']})",
                                        finish_reason="stop"
                                    )
                                else:
                                    yield self._create_stream_chunk(
                                ...
                                if url_mode:
                                    yield self._create_completion_response(
                                        generation_result["generated_assets"]["upscaled_image"]["url"],
                                        media_type="image"
                                    )
                                else:
                                    yield self._create_completion_response(
            ...
            # 返回结果
            # 存储URL用于日志记录
            generation_result["generated_url"] = local_url
            generation_result["generated_assets"] = {
                "type": "image",
                "origin_image_url": image_url,
